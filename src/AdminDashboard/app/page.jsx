...
import { Activity, Clock, ShieldAlert, Shield } from 'lucide-react';
  ...
  return (
    <div className="flex bg-[#050505] min-h-screen text-white font-sans selection:bg-blue-500/30">
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden relative">
        <AppHeader route={['Dashboard', 'Overview']} />
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between bg-[#111111] p-4 rounded-xl border border-[#2A2A2A] shadow-md gap-4">
                <div className="flex items-center space-x-6">
                    <div className="flex items-center space-x-3">
                        ...
                        <div>
                            <p className="text-[10px] text-gray-500 font-bold uppercase tracking-wider">Active Agents</p>
                            <p className="text-xl font-bold text-green-400">{agentStats.active}</p>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center space-x-3">
                        ...
                        <div>
                            <p className="text-[10px] text-gray-500 font-bold uppercase tracking-wider">Disconnected</p>
                            <p className="text-xl font-bold text-red-500">{agentStats.offline}</p>
                        </div>
                    </div>
                </div>
                        ...
                        <div className="grid grid-cols-2 gap-y-2 gap-x-2 mb-4 max-h-24 overflow-y-auto scrollbar-hide">
                            {alertTypeData.map((item, idx) => (
                                <div key={item.name} className="flex items-center space-x-2">
                                    ...
                                    <span className="truncate text-[11px] font-bold text-gray-300" title={item.name}>{item.name}</span>
                                </div>
                            ))}
                        </div>
                        ...
                        <table className="w-full text-left text-sm border-collapse">
                            <thead className="sticky top-0 bg-[#111111] z-10">
                                <tr className="border-b border-[#2A2A2A] text-gray-500 font-mono text-[10px] uppercase tracking-wider">
                                    <th className="py-3 font-bold w-3/5">Rule Description</th>
                                    <th className="py-3 font-bold text-center">Level</th>
                                    <th className="py-3 font-bold text-right">Count</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-[#1A1A1A]">
                                {tableData.length > 0 ? tableData.map((event, idx) => (
                                    ...
                                    <tr>
                                        <td colSpan="3" className="py-12 text-center text-gray-500 font-mono text-xs">Không có cảnh báo nghiêm trọng nào trong mốc thời gian này</td>
                                    </tr>
                                )}
                            </tbody>
                        </table>
