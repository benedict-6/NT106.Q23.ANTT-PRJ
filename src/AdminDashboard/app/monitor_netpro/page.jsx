...
const NetproPage = () => {
  ...
  useEffect(() => {
    if (isConnected) {
      ...
      fetchDbLogsViaSocket('FETCH_NETPRO_LOGS', agentId, isLive ? undefined : timeRange, isLive ? 1 : page);
    }
                      ...
                      onClick={async () => {
                        ...
                        try {
                          const res = await fetch(`${process.env.NEXT_PUBLIC_MASTER_URL}/api/dashboard/export`, {
                            ...
                            body: JSON.stringify({ agents: agentId === 'all' ? 'all' : [agentId], timeRange, dataTypes: ['net_pro'] })
                          });
                          ...
                          a.href = url;
                          a.download = `net_pro_${agentId}_${timeRange}.zip`;
                          a.click();
                        } catch (err) {
