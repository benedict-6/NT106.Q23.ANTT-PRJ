...
const NetproPage = () => {
  ...
  // Gửi request lấy historical db logs khi agentId thay đổi, socket kết nối, hoặc đổi chế độ live / khoảng thời gian / trang
  useEffect(() => {
    if (agentId && isConnected) {
      setDbLogs([]); // Clear old state
      fetchDbLogsViaSocket('FETCH_SYSLOGS', agentId, isLive ? undefined : timeRange, isLive ? 1 : page);
    }
                      ...
                      onClick={async () => {
                        ...
                        try {
                          const res = await fetch(`${process.env.NEXT_PUBLIC_MASTER_URL}/api/dashboard/export`, {
                            ...
                            body: JSON.stringify({ agents: agentId === 'all' ? 'all' : [agentId], timeRange, dataTypes: ['log_monitoring'] })
                          });
                          ...
                          a.href = url;
                          a.download = `log_monitoring_${agentId}_${timeRange}.zip`;
                          a.click();
                        } catch (err) {
