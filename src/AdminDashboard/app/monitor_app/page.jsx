...
const ApplicationsPage = () => {
  ...
  // Reset trang khi đổi các bộ lọc hoặc đổi trạng thái Live/Offline
  useEffect(() => {
    setPage(1);
  }, [agentId, isLive, timeRange]);

  // Gửi request lấy historical db applications khi agentId thay đổi, socket kết nối, hoặc đổi chế độ live / khoảng thời gian / trang
  useEffect(() => {
    if (agentId && isConnected) {
      setDbLogs([]); // Clear old state
      fetchDbLogsViaSocket('FETCH_APPLICATIONS', agentId, isLive ? undefined : timeRange, isLive ? 1 : page);
    }
  }, [agentId, isConnected, isLive, timeRange, page, fetchDbLogsViaSocket, setDbLogs]);
                      ...
                      onClick={async () => {
                        ...
                        try {
                          const res = await fetch(`${process.env.NEXT_PUBLIC_MASTER_URL}/api/dashboard/export`, {
                            ...
                            body: JSON.stringify({ agents: agentId === 'all' ? 'all' : [agentId], timeRange, dataTypes: ['applications'] })
                          });
                          ...
                          a.href = url;
                          a.download = `applications_${agentId}_${timeRange}.zip`;
                          a.click();
                        } catch (err) {
